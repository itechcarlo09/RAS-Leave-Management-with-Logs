...
namespace RAS_Leave_Management
{
    public partial class Manager : MaterialSkin.Controls.MaterialForm
    {
        ...
        private void leave_request()
        {
            ...
            SqlCommand cm = cn.CreateCommand();
            cm.CommandType = CommandType.Text;
            cm.CommandText = "SELECT leave_id 'Leave ID', acc_lastname + ', ' + acc_firstname 'Name', CONVERT(VARCHAR(12), leave_request_date, 107) 'Date', CASE leave_span when .5 THEN 'Half Day' WHEN 1 THEN 'Whole Day' END AS 'Leave Span', leave_reason 'Reason', leave_type 'Type' FROM RequestLeave RL INNER JOIN Account A ON RL.leave_request = A.acc_id Where leave_status = 2 AND RL.leave_respond = '"+ Login.id +"' AND leave_request_date > '"+ DateTime.Now +"'"; 
            cm.ExecuteNonQuery();
        ...
        private void GetEmployees()
        {
            ...
            SqlCommand cm = cn.CreateCommand();
            cm.CommandType = CommandType.Text;
            cm.CommandText = "SELECT acc_lastname + ', ' + acc_firstname 'Name' FROM Account Where acc_leader = '" + Login.id + "'";
            cm.ExecuteNonQuery();
            DataTable dt = new DataTable();
            SqlDataAdapter da = new SqlDataAdapter(cm);
            da.Fill(dt);
        ...
        private void GetLeaveUpdate()
        {
            ...
            SqlCommand cm = cn.CreateCommand();
            cm.CommandType = CommandType.Text;
            cm.CommandText = "SELECT CONVERT(VARCHAR(12), leave_request_date, 107) 'Date', CASE leave_span when .5 THEN 'Half Day' WHEN 1 THEN 'Whole Day' END AS 'Leave Span', leave_reason 'Reason',leave_type 'Type', CASE leave_status when 1 then 'Approved' when 0 then 'Rejected' END AS 'Status' FROM RequestLeave RL INNER JOIN Account A ON RL.leave_request = A.acc_id Where acc_id = '" + Login.id + "' AND leave_status != 2 AND leave_request_date > '" + DateTime.Now + "'";
            cm.ExecuteNonQuery();
            DataTable dt = new DataTable();
            SqlDataAdapter da = new SqlDataAdapter(cm);
            da.Fill(dt);
