...
namespace RAS_Leave_Management
{
    public partial class TopManagement : MaterialSkin.Controls.MaterialForm
    {
        ...
        private void leave_request()
        {
            ...
            cm.CommandText = "SELECT leave_id 'Leave ID', acc_lastname + ', ' + acc_firstname 'Name', CONVERT(VARCHAR(12), leave_request_date, 107) 'Date', CASE leave_span when .5 THEN 'Half Day' WHEN 1 THEN 'Whole Day' END AS 'Leave Span', leave_reason 'Reason', leave_type 'type' FROM RequestLeave RL INNER JOIN Account A ON RL.leave_request = A.acc_id Where leave_status = 2 AND RL.leave_respond = '" + Login.id + "'";
            cm.ExecuteNonQuery();
