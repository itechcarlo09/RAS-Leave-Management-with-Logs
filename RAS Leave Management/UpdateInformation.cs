...
namespace RAS_Leave_Management
{
    public partial class UpdateInformation : MaterialSkin.Controls.MaterialForm
    {
        ...
        {
            InitializeComponent();
            dateBirthdate.MaxDate = DateTime.Now.AddYears(-18);
        }
            ...
            using (SqlCommand command = new SqlCommand("SELECT * FROM Account Where acc_id = '"+ ManagePeople.ID +"'", cn))
            {
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        dateBirthdate.Value = Convert.ToDateTime(reader.GetValue(4));
                        if (Convert.ToString(reader.GetValue(5)) == "Male")
                            radioMale.Checked = true;
                        else
                            radioFemale.Checked = true;
                        if (Convert.ToString(reader.GetValue(12)) == "New")
                            radioNew.Checked = true;
                        else
                            radioRegular.Checked = true;
                        manager = Convert.ToInt32(reader.GetValue(11));
                        if (Convert.ToString(reader.GetValue(9)) == "Manager")
                        {
        ...
        private void btnUpdateEmployees_Click(object sender, EventArgs e)
        {
            ...
            {
            if(txtFirstName.Text != null && txtLastName.Text != null && dateBirthdate.Value != null && radioFemale.Checked == true || radioMale.Checked == true && radioNew.Checked == true || radioRegular.Checked == true)
            {
                ...
                if(identifier == "Manager")
                {
                    if(demoteflag == 1)
                    {
                        if(ManagePeople.IDEmp.Count == 0)
                        {
                            ...
                            cm.CommandText = "UPDATE Account SET acc_firstname = '" + txtFirstName.Text + "', acc_middlename = '" + txtMiddleName.Text + "', acc_lastname = '" + txtLastName.Text + "', acc_birthday = '" + dateBirthdate.Value + "', acc_sex = '" + gender + "', acc_position = '" + status + "', acc_leader = '" + IDs[comboManager.SelectedIndex] + "', acc_type = 'Employee' WHERE acc_id = '" + ManagePeople.ID + "'";
                            cm.ExecuteNonQuery();
                            cn.Close();
                        }
                        else
                    ...
                    else
                    {
                        ...
                        SqlCommand cm = cn.CreateCommand();
                        cm.CommandType = CommandType.Text;
                        cm.CommandText = "UPDATE Account SET acc_firstname = '" + txtFirstName.Text + "', acc_middlename = '" + txtMiddleName.Text + "', acc_lastname = '" + txtLastName.Text + "', acc_birthday = '" + dateBirthdate.Value + "', acc_sex = '" + gender + "', acc_position = '" + status + "' WHERE acc_id = '" + ManagePeople.ID + "'";
                        cm.ExecuteNonQuery();
                        cn.Close();
                    }
                }
                else
                {
                    if (promoteflag == 1)
                    {
                        ...
                        SqlCommand cm = cn.CreateCommand();
                        cm.CommandType = CommandType.Text;
                        cm.CommandText = "UPDATE Account SET acc_firstname = '" + txtFirstName.Text + "', acc_middlename = '" + txtMiddleName.Text + "', acc_lastname = '" + txtLastName.Text + "', acc_birthday = '" + dateBirthdate.Value + "', acc_sex = '" + gender + "', acc_position = '" + status + "', acc_leader = '1', acc_type = 'Manager' WHERE acc_id = '" + ManagePeople.ID + "'";
                        cm.ExecuteNonQuery();
                        cn.Close();
                    }
                    else
        ...
        private void provisionalupdate()
        {
            ...
            if(newbie == 1)
            {
                SqlCommand cm = cn.CreateCommand();
                cm.CommandType = CommandType.Text;
                cm.CommandText = "UPDATE Account SET acc_firstname = '" + txtFirstName.Text + "', acc_middlename = '" + txtMiddleName.Text + "', acc_lastname = '" + txtLastName.Text + "', acc_birthday = '" + dateBirthdate.Value + "', acc_sex = '" + gender + "', acc_position = '" + status + "', acc_leader = '" + IDs[comboManager.SelectedIndex] + "', acc_leave = 7.5 WHERE acc_id = '" + ManagePeople.ID + "'";
                cm.ExecuteNonQuery();
            }
            else
            {
                SqlCommand cm = cn.CreateCommand();
                cm.CommandType = CommandType.Text;
                cm.CommandText = "UPDATE Account SET acc_firstname = '" + txtFirstName.Text + "', acc_middlename = '" + txtMiddleName.Text + "', acc_lastname = '" + txtLastName.Text + "', acc_birthday = '" + dateBirthdate.Value + "', acc_sex = '" + gender + "', acc_position = '" + status + "', acc_leader = '" + IDs[comboManager.SelectedIndex] + "' WHERE acc_id = '" + ManagePeople.ID + "'";
                cm.ExecuteNonQuery();
            }
        }
    }
